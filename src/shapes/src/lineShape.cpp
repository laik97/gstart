...
namespace shape
{
  ...
  {
    return sf::Vector2f(
        this->points_[0].position.x + this->points_[1].position.x,
  ...
  void LineShape::rotate(const float radiusAngle)
  {
    const auto currentCenter = this->center();
    const auto currentXAngle = this->xAxisAngle();
  }
